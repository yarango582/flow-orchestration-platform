...
export class MongoDBOperationsNode extends BaseNode<MongoDBInput, MongoDBOutput, MongoDBConfig> {
  ...
  async execute(input: MongoDBInput): Promise<NodeResult<MongoDBOutput>> {
    ...
    const timeout = input.options?.timeout || this.config.defaultTimeout || 30000
  ...
  private async connectWithRetry(connectionString: string, retries: number): Promise<void> {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        this.client = new MongoClient(connectionString, {
          ...
          serverSelectionTimeoutMS: this.config.connectionPool?.serverSelectionTimeoutMS || 5000
        })
  ...
  private async executeOperation(
    ...
    const { operation, query = {}, document, update, pipeline, options = {} } = input
    ...
    switch (operation) {
      case 'find':
        ...
        const findResult = await collection.find(processedQuery, findOptions).toArray()
        return {
          result: findResult,
      ...
      case 'findOne':
        ...
        const findOneResult = await collection.findOne(processedQuery, findOneOptions)
        return {
  ...
  validate(input: MongoDBInput): boolean {
    ...
    if (!input.connectionString || !input.database || !input.collection || !input.operation) {
      return false
    }
    ...
    switch (input.operation) {
      ...
      case 'insertMany':
        return !!input.document && Array.isArray(input.document) && input.document.length > 0
      ...
      case 'aggregate':
        return !!input.pipeline && Array.isArray(input.pipeline) && input.pipeline.length > 0
